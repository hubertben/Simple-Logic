...
public void create(){
  ...
  for(int i = 0; i < (int)pow(2, inputs_inUse.size()); i++){
    ...
    for(int j = 0; j < inputs_inUse.size(); j++){
       
       if(o.charAt(j) == '0'){
         inputs_inUse.get(j).state = false;
       }else{
         inputs_inUse.get(j).state = true;
       }
    }
    

    for(int l = 0; l < inputs_inUse.size(); l++){
      System.out.print(inputs[l].state + "\t");
    }
    println();
    
        for(int q = 0; q < connections.size(); q++){
          connections.get(q).update();
          for(int b = 0; b < board.size(); b++){
            Circuit cur = board.get(b);
            cur.update_signal(); 
          }
  ...
  ArrayList <Pin> in_ = new ArrayList<Pin>();
  ArrayList <Pin> out_ = new ArrayList<Pin>();
  
  for(int q = 0; q < connections.size(); q++){
     if(connections.get(q).source.condition && connections.get(q).source.type){
       
       if(!in_.contains(connections.get(q).source)){
         in_.add(connections.get(q).source);
         in_counter++;
       }
     ...
     if(connections.get(q).destination.condition && !connections.get(q).destination.type){
       
       
       
         if(!out_.contains(connections.get(q).destination)){
           out_.add(connections.get(q).destination);
           out_counter++;
         }
  ...
  storage.storage.add(new Circuit_Display(new_circuit_name, 0, 0, in_counter, out_counter, bitboard));
  new_circuit_name = "";
